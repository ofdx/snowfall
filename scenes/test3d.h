...
public:
	...
	void receive_data(string const& data) override;
	void draw(int ticks);

};


TestScene3D::TestScene3D(Scene::Controller *ctrl) :
	Scene3D(ctrl),
...
void TestScene3D::receive_data(string const& data){
	if(data.empty())
		return;
...
void TestScene3D::draw(int ticks){
	m_tickcounter += ticks;
